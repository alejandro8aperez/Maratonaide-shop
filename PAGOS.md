...
2. Configura cada link con el nombre de la edición, el precio definitivo, sin
   dirección de envío y con un SKU distinto (`maratonaide-es`,
   `maratonaide-en`, `maratonaide-fr`). Usa un link reutilizable.
3. Reemplaza las tres URLs de `PAYMENT_LINKS` en `js/checkout.js`. Aunque las
   ediciones tengan el mismo precio, los enlaces distintos simplifican la
   conciliación y la entrega correcta del idioma.
...
2. Comprobar que la transacción está `APPROVED`, corresponde al monto y SKU de
   MARATONaide y no se ha procesado antes.
